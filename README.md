...
- 顶部 `ORIGIN / SCREEN / A4` 用于选择红色任务模式，当前模式以绿色边框高亮；底部仍是重新识别、开始/继续和暂停。
- 屏幕正方形锁定后会持续保留绿色边框和白色原点标记，切换到 A4 模式也不会消失；A4 矩形使用独立蓝色边框，不会把 A4 中心误画成题目原点。
...
- 默认模式为 A4 巡边。STM32 或顶部触摸按钮可用同一 `TYPE=0x13`、`INDEX=0/1/2` 选择原点复位、屏幕正方形巡边或 A4 巡边。
- 模式切换不会清除已锁定几何。原点与屏幕巡边共享屏幕正方形缓存，A4 使用独立缓存；进入已有缓存的模式时直接恢复航点，只有底部 `REACQUIRE` 或 `TYPE=0x12` 才清除并重新识别当前模块。
...
- 触摸区提供三个模式按钮和 `REACQUIRE`、`START/RESUME`、`PAUSE`，与 STM32 控制帧共用同一状态机。
...
红色视觉正常运行时输出 `01/02/03/04`；红色主控可发送 `10/11/12/13`。摄像头触摸按钮会把对应的 `10/11/12/13` 原样回报给红色主控，用于同步模式和立即制动，因此这些控制帧在红色链路上是双向的。绿色链路只需要 `02/04/05` 三种视觉输出，绿色主控发送 `10/11`。两条链路物理独立，不需要把所有 TYPE 放进同一个业务状态机。
